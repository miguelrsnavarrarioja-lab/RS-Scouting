...
(function () {
  ...
  function deleteClubPermanently(clubId) {
    ...
    const cObj = (state.directory.clubes || []).find(c => c.id === clubId || String(c.codigo) === String(clubId));

    if (!state.directory.deletedClubIds.includes(String(clubId))) {
      state.directory.deletedClubIds.push(String(clubId));
    }
    if (cObj) {
    ...
    if (cObj && cObj.id && cObj.id !== clubId) {
      deleteFromFirebase('clubes', cObj.id);
    }

    if (Array.isArray(state.directory.clubes)) {
      state.directory.clubes = state.directory.clubes.filter(c => c.id !== clubId && String(c.codigo) !== String(clubId));
    }
