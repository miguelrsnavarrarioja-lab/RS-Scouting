...
function processEquiposImport(text) {
    ...
    const defaultTemporada = document.getElementById('importerEquipoTemporada')?.value || '';
    const defaultCategoria = document.getElementById('importerEquipoCategoria')?.value || '';
    const defaultCompeticion = document.getElementById('importerEquipoCompeticion')?.value || '';
    ...
    while (i < lines.length) {
        ...
        if (line.match(/^\d+$/)) {
            i++;
            if (i < lines.length) {
                // Eliminar puntos como pidió el usuario
                let nombreBruto = lines[i].replace(/^\d+\s+/, '').replace(/(?:\s+\d+)+$/, '').replace(/\[|\]/g, '').replace(/\./g, '').split('(')[0].trim();
                stagedExcelRows.push(createEquipoObj(nombreBruto, defaultCategoria, defaultTemporada, defaultCompeticion));
            }
        } else {
            let nombreBruto = line.replace(/^\d+\s+/, '').replace(/(?:\s+\d+)+$/, '').replace(/\[|\]/g, '').replace(/\./g, '').split('(')[0].trim();
            stagedExcelRows.push(createEquipoObj(nombreBruto, defaultCategoria, defaultTemporada, defaultCompeticion));
        }
        i++;
    }
...
function createEquipoObj(clubBase, cat, temp, comp) {
    return {
        _checked: true,
...
function saveExcelToDirectory() {
    ...
    if (currentImporterType === 'equipos') {
        ...
        toSave.forEach(r => {
            window.state.directory.equipos.push({
        ...
        toSave.forEach(r => {
            window.state.directory.clubes.push({
        ...
        toSave.forEach(r => {
            window.state.directory.plantillas.push({
